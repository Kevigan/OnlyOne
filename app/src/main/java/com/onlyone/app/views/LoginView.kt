...
import com.onlyone.app.utils.applyAppLocale
...
import com.onlyone.app.views.settingsView.LanguageDropdown
...
fun LoginView(
    ...
    // ────────────────────────────────────────────────────────────────────────────
    // App language picker data
    // ────────────────────────────────────────────────────────────────────────────
    val availableLanguages = listOf("English", "Deutsch")
    val languageMap = mapOf("en" to "English", "de" to "Deutsch")
    ...
    Column(
        ...
        LanguageDropdown(
            ...
            if (!pickerEnabled || code == currentAppLang) return@LanguageDropdown
            userViewModel.setAppLanguageAndApply(code)   // 👈 single call
            nextSwitchAtMs = android.os.SystemClock.uptimeMillis() + 3000L
        }
